...
public:
  ...
  CreateSubscriptionFromConfigStr(const std::string &config) {
    ...
    bool GetIsObeliskMsg = GetIsObeliskMsg(config_map);

    auto sub = create_subscription<obelisk_control_msgs::msg::PositionSetpoint>(
        topic, depth,
        std::bind(&ObeliskNode::TestCallback, this, std::placeholders::_1));
    return sub;
