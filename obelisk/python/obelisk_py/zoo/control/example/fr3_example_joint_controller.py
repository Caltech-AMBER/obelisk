...
class FR3ExampleJointController(ObeliskController):
    ...
    def compute_control(self) -> Type:
        ...
        position_setpoint_msg.u_mujoco = [(0.3 * np.sin(self.t)) for _ in range(7)]  # example state-independent input
        position_setpoint_msg.q_des = [(0.3 * np.sin(self.t)) for _ in range(7)]  # example state-independent input
